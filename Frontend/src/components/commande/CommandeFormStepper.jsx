...
const steps = [
  ...
  { id: 2, name: "Récapitulatif", icon: FileText, description: "Vérification et validation" },
];

const CommandeFormStepper = ({ formData, onInputChange, onSubmit, onClose, isSubmitting, editingCommande, clients, produits }) => {
  ...
  const handleNext = () => {
    if (step === 0) {
      ...
      if (!formData.produits || formData.produits.length === 0) {
        setErrors({ produits: "Ajoutez au moins un produit" });
        return;
      }
      setErrors({});
      setStep(step + 1);
  ...
  const handleChange = (path, value) => {
    ...
    if (path === 'produits') setErrors(prev => ({ ...prev, produits: undefined }));
  };
  ...
  const handleProductChange = useCallback((index, field, value) => {
    ...
    switch (field) {
      case 'produit':
        ...
        if (produit) {
          current.prixHT = produit.prixVente || produit.prix || 0;
    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/40 backdrop-blur-md p-4">
      <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-3xl bg-white/95 shadow-2xl backdrop-blur-sm animate-in zoom-in-95 duration-300 border border-white/20 relative">
        ...
        <div className="p-6">
                          ...
                          >
                            ...
                            {produits.map(p => (
                              <option key={p._id} value={p._id}>
                                {p.designation} - {(p.prixVente || p.prix || 0).toLocaleString()} DH HT
                              </option>
                            ))}
                          </select>
                        </div>
          ...
          {step === 2 && (
            <div className="rounded-xl bg-gradient-to-br from-[#e2f0d6]/10 to-white p-5 border border-[#d4e6b0] animate-in slide-in-from-right-5 duration-300">
              <h3 className="font-semibold text-[#5a7c3c] mb-4 flex items-center gap-2">
                <FileText size={16} /> Récapitulatif de la commande
              </h3>
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="inline-flex items-center rounded-full bg-[#e2f0d6] px-2 py-0.5 text-xs font-medium text-[#5a7c3c]">En cours</span>
                </div>
                <div className="border-t border-[#d4e6b0] pt-3 mt-2">
                  <div className="flex justify-between font-bold">
                    <span>Total TTC :</span>
                    <span className="text-xl text-[#5a7c3c]">{calculateTotal.toLocaleString()} DH</span>
                  </div>
                </div>
                {(formData.produits || []).length > 0 && (
                  <div className="mt-3 text-xs text-slate-400 space-y-1">
                    <p className="font-medium">Détail des produits :</p>
                    {formData.produits.map((prod, idx) => {
                      const prodDetail = produits.find(p => p._id === prod.produit);
                      return (
                        <div key={idx} className="flex justify-between">
                          <span>{prodDetail?.designation || 'Produit'} x {prod.quantite}</span>
                          <span>{prod.total.toLocaleString()} DH</span>
                        </div>
                      );
                    })}
                  </div>
        ...
        <div className="flex items-center justify-between border-t border-gray-100 px-6 py-4 bg-white/80">
          ...
          {step < 2 ? (
            ...
            >
              ...
              {isSubmitting ? "Enregistrement..." : (editingCommande ? "Mettre à jour" : "Créer la commande")}
            </button>
          )}
        </div>
      </div>
...
export default CommandeFormStepper;